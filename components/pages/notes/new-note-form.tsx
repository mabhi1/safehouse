...
export default function NewNoteForm({ userId }: { userId: string }) {
  ...
  const { formValues, handleInputChange, handleSubmit, isPending, isValid } = useFormSubmit<NewNoteFormValues>({
    initialValues: initialFormValues,
    onSubmit: async (values) => {
      const result = await addNote(values.title.trim(), values.description.trim(), userId);
      if (result.data) {
        ...
        <div className="flex flex-col gap-5">
          <div className="flex items-center justify-between lg:gap-20">
            ...
            <div className="flex items-center gap-2 md:gap-5">
              <Button type="button" variant="secondary" ICON={CircleSlash} onClick={() => router.back()} mobileVariant>
                Cancel
              </Button>
